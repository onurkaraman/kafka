...
private[kafka] class ControllerThread(kafkaConfig: KafkaConfig, zkUtils: ZkUtils, time: Time, metrics: Metrics)
  extends ShutdownableThread(name = s"controller-thread-${kafkaConfig.brokerId}") with Logging {
  ...
  private val _topicCreationListener = new TopicCreationListener(this)
  private var _topicExpansionListeners = Map.empty[String, TopicExpansionListener]
  private val _topicDeletionListener = new TopicDeletionListener(this)
  private val _partitionReassignmentListener = new PartitionReassignmentListener(this)
  private var _partitionReassignmentIsrChangeListeners = Map.empty[TopicAndPartition, PartitionReassignmentIsrChangeListener]
  private val _isrChangeNotificationListener = new IsrChangeNotificationListener(this)
  private val _preferredReplicaLeaderElectionListener = new PreferredReplicaLeaderElectionListener(this)
  private val _brokerChangeListener = new BrokerChangeListener(this)
  private val _controllerChangeListener = new ControllerChangeListener(this)
  ...
  def enqueueTopicCreation(topics: Set[String]): Unit = {
    _queue.put(TopicCreation(topics))
  }
  def enqueueTopicExpansion(assignments: Map[TopicAndPartition, Seq[Int]]): Unit = {
    _queue.put(TopicExpansion(assignments))
  }
  def enqueueTopicDeletion(topics: Set[String]): Unit = {
    _queue.put(TopicDeletion(topics))
  }
  def enqueueTopicDeletionDeleteReplicaResult(id: Int, abstractResponse: AbstractResponse): Unit = {
    _queue.put(TopicDeletionDeleteReplicaResult(id, abstractResponse))
  }
  def enqueuePartitionReassignment(partition: TopicAndPartition, replicas: Seq[Int]): Unit = {
    _queue.put(PartitionReassignment(partition, replicas))
  }
  def enqueuePartitionReassignmentIsrChange(partition: TopicAndPartition, isr: Seq[Int]): Unit = {
    _queue.put(PartitionReassignmentIsrChange(partition, isr))
  }
  def enqueueIsrChangeNotification(sequenceNumbers: Seq[String]): Unit = {
    _queue.put(IsrChangeNotification(sequenceNumbers))
  }
  def enqueuePreferredReplicaLeaderElection(partition: TopicAndPartition): Unit = {
    _queue.put(PreferredReplicaLeaderElection(partition))
  }
  def enqueueAutoPreferredReplicaLeaderElection(): Unit = {
    _queue.put(AutoPreferredReplicaLeaderElection)
  }
  def enqueueBrokerChange(brokers: Set[Int]): Unit = {
    _queue.put(BrokerChange(brokers))
  }
  def enqueueControlledShutdown(id: Int): Set[TopicAndPartition] = {
    val resultQueue = new LinkedBlockingQueue[Set[TopicAndPartition]]
    _queue.put(ControlledShutdown(id, resultQueue))
    resultQueue.take()
  }
  def enqueueStartup(): Unit = {
    _queue.put(Startup)
  }
  def enqueueElect(): Unit = {
    _queue.put(Elect)
  }
  def enqueueResign(newController: Int): Unit = {
    _queue.put(Resign(newController))
  }

  def parseControllerId(controllerString: String) = {
    try {
  ...
  def fetchControllerId = zkUtils.readDataMaybeNull(ZkUtils.ControllerPath)._1 match {
  ...
  case class TopicCreation(topics: Set[String]) extends ControllerEvent {
    ...
    private def createTopic(topic: String): Unit = {
      val topicExpansionListener = new TopicExpansionListener(ControllerThread.this, zkUtils, topic)
  ...
  case class PartitionReassignment(partition: TopicAndPartition, replicas: Seq[Int]) extends ControllerEvent {
    ...
    private def registerPartitionReassignmentIsrChangeListener(partition: TopicAndPartition): Unit = {
      val partitionReassignmentIsrChangeListener = new PartitionReassignmentIsrChangeListener(ControllerThread.this, zkUtils, partition)
...
class TopicCreationListener(controllerThread: ControllerThread) extends IZkChildListener {
  import scala.collection.JavaConverters._
  override def handleChildChange(parentPath: String, currentChilds: java.util.List[String]): Unit = {
    controllerThread.enqueueTopicCreation(currentChilds.asScala.toSet)
  }
}

class TopicExpansionListener(controllerThread: ControllerThread, zkUtils: ZkUtils, topic: String) extends IZkDataListener {
  override def handleDataChange(dataPath: String, data: scala.Any): Unit = {
    controllerThread.enqueueTopicExpansion(zkUtils.getReplicaAssignmentForTopics(Seq(topic)).toMap)
  }

  override def handleDataDeleted(dataPath: String): Unit = {}
}

class TopicDeletionListener(controllerThread: ControllerThread) extends IZkChildListener {
  import scala.collection.JavaConverters._
  override def handleChildChange(parentPath: String, currentChilds: java.util.List[String]): Unit = {
    controllerThread.enqueueTopicCreation(currentChilds.asScala.toSet)
  }
...
class PartitionReassignmentListener(controllerThread: ControllerThread) extends IZkDataListener {
  override def handleDataChange(dataPath: String, data: scala.Any): Unit = {
    val assignments = ZkUtils.parsePartitionReassignmentData(data.toString)
    assignments.foreach { case (partition, replicas) => controllerThread.enqueuePartitionReassignment(partition, replicas) }
  }

  override def handleDataDeleted(dataPath: String): Unit = {}
}

class PartitionReassignmentIsrChangeListener(controllerThread: ControllerThread, zkUtils: ZkUtils, partition: TopicAndPartition) extends IZkDataListener {
  override def handleDataChange(dataPath: String, data: scala.Any): Unit = {
    val leaderAndIsrOpt = zkUtils.getLeaderAndIsrForPartition(partition.topic, partition.partition)
    leaderAndIsrOpt.foreach(leaderAndIsr => controllerThread.enqueuePartitionReassignmentIsrChange(partition, leaderAndIsr.isr))
  }

  override def handleDataDeleted(dataPath: String): Unit = {}
}

class IsrChangeNotificationListener(controllerThread: ControllerThread) extends IZkChildListener {
  import scala.collection.JavaConverters._
  override def handleChildChange(parentPath: String, currentChilds: java.util.List[String]): Unit = {
    controllerThread.enqueueIsrChangeNotification(currentChilds.asScala)
  }
}

class PreferredReplicaLeaderElectionListener(controllerThread: ControllerThread) extends IZkDataListener {
  override def handleDataChange(dataPath: String, data: scala.Any): Unit = {
    val partitions = PreferredReplicaLeaderElectionCommand.parsePreferredReplicaElectionData(data.toString)
    partitions.foreach(partition => controllerThread.enqueuePreferredReplicaLeaderElection(partition))
  }

  override def handleDataDeleted(dataPath: String): Unit = {}
}

class BrokerChangeListener(controllerThread: ControllerThread) extends IZkChildListener {
  import scala.collection.JavaConverters._
  override def handleChildChange(parentPath: String, currentChilds: java.util.List[String]): Unit = {
    controllerThread.enqueueBrokerChange(currentChilds.asScala.map(_.toInt).toSet)
  }
}

class ControllerChangeListener(controllerThread: ControllerThread) extends IZkDataListener {
  override def handleDataChange(dataPath: String, data: scala.Any): Unit = {
    controllerThread.enqueueResign(controllerThread.parseControllerId(data.toString))
  }

  override def handleDataDeleted(dataPath: String): Unit = {
    controllerThread.enqueueResign(controllerThread.fetchControllerId)
    controllerThread.enqueueElect()
  }
}